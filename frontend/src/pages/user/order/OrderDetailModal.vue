        ...
        <div class="custom-modal-content shadow-lg border-0 rounded-4 slide-up d-flex flex-column"
            ...
            <div
                ...
                <button v-if="order?.status === 'delivered' && (!order?.reviews || order?.reviews.length === 0)"
                    @click="handleReview" class="btn btn-sm fw-bold px-3 text-uppercase text-white border-0"
                    style="background-color: #E6C97A; border-radius: 8px;">
                ...
                <button v-if="order?.status === 'delivered' && order?.reviews && order?.reviews.length > 0"
                    ...
                    class="btn btn-sm fw-bold px-3 text-uppercase text-white border-0"
                    style="background-color: #009981; border-radius: 8px;">
                ...
                <button v-if="order?.status === 'delivered'" @click="handleReturn"
                    class="btn btn-sm fw-bold px-3 text-uppercase text-white border-0"
                    style="background-color: #f08c00; border-radius: 8px;">
                ...
                <button v-if="order?.id" @click="handleDownloadInvoice"
                    class="btn btn-sm fw-bold px-3 text-uppercase text-white border-0"
                    style="background-color: #9F273B; border-radius: 8px;">
                ...
                <button v-if="['delivered', 'cancelled', 'returned', 'return_requested'].includes(order?.status)"
                    @click="handleReorder" class="btn btn-sm fw-bold px-4 text-uppercase  border-0"
                    style="background-color: #E2F3E5; color: #009981; border-radius: 8px;">
                    <i class="bi bi-cart-plus me-1"></i> Mua Lại
                </button>

                <!-- Đóng -->
                <button @click="$emit('close')" class="btn btn-sm fw-bold px-4 ms-md-2 text-uppercase border-0"
                    style="background-color: #f1f3f5; color: #495057; border-radius: 8px;">
                    Đóng
                </button>
            </div>
