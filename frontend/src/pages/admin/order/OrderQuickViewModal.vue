    ...
    <div class="modal fade" id="quickViewOrderModal" tabindex="-1" aria-hidden="true" style="z-index: 1060;"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div id="invoice-printable" class="modal-content rounded-4 border-0 shadow">
                <div
                    class="modal-header border-bottom pb-3 bg-light rounded-top-4 d-flex justify-content-between align-items-center">
                    <h5 class="fw-bold text-dark mb-0 flex-grow-1 d-flex align-items-center">
                        <i class="bi bi-receipt text-brand me-2"></i>Đơn Hàng
                        <span class="text-brand font-monospace ms-1" v-if="currentOrder">{{ currentOrder.order_code
                            }}</span>
                        <div v-if="isFetching" class="spinner-border spinner-border-sm text-brand ms-3" role="status">
                        </div>
                    </h5>
                    ...
                    <div class="d-flex align-items-center gap-2 gap-md-3 no-print">
                        ...
                        <select v-if="currentOrder"
                            ...
                            style="width: auto; min-width: 140px; border-width: 2px;" v-model="localSelectedWarehouseId"
                        ...
                        <button v-if="currentOrder" type="button"
                    ...
                    <button type="button"
                        class="btn btn-sm btn-danger text-white rounded action-btn-hover ms-2 no-print d-flex align-items-center justify-content-center"
                        style="width: 32px; height: 32px; padding: 0;" @click="hide" title="Đóng">
                ...
                <div class="modal-body p-4 bg-white" v-if="currentOrder">
                    <div class="row g-4">
                        <div class="col-lg-4 border-end">
                            <h6 class="fw-bold text-muted text-uppercase mb-3"><i
                                    class="bi bi-person-badge me-2 text-brand"></i>Người nhận</h6>
                            <div class="mb-2"><span class="text-muted fw-semibold">Họ tên:</span> <strong
                                    class="text-dark float-end">{{ currentOrder.customer_name }}</strong></div>
                            <div class="mb-2"><span class="text-muted fw-semibold">Điện thoại:</span> <strong
                                    class="text-dark float-end">{{ currentOrder.customer_phone }}</strong></div>
                            <div class="mb-3">
                                <span class="text-muted fw-semibold d-block mb-1">Địa chỉ giao hàng:</span>
                                <div class="p-2 bg-light border rounded small fw-medium">{{
                                    currentOrder.customer_address }}</div>
                            </div>
                            <div class="mb-4">
                                <span class="text-muted fw-semibold d-block mb-1">Ghi chú của khách:</span>
                                <div
                                    class="p-2 bg-warning bg-opacity-10 text-dark fw-medium border border-warning rounded small fst-italic">
                                    {{ currentOrder.order_note || 'Không có ghi chú' }}</div>
                            </div>

                            <h6 class="fw-bold text-muted text-uppercase mb-3 border-top pt-4"><i
                                    class="bi bi-credit-card me-2 text-brand"></i>Thanh toán</h6>
                            ...
                            <div class="mb-2 d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge shadow-sm border"
                                    :class="getPaymentBadge(currentOrder.payment_status)">{{
                                        formatPaymentStatus(currentOrder.payment_status) }}</span>
                            </div>
                        </div>

                        <div class="col-lg-8">
                            <h6 class="fw-bold text-muted text-uppercase mb-3"><i
                                    class="bi bi-box-seam me-2 text-brand"></i>Sản phẩm đã đặt</h6>
                                        ...
                                        <tr v-for="item in currentOrder.items" :key="item.id">
                                            <td class="ps-3 py-3">
                                                <div class="d-flex align-items-center gap-2">
                                                    <img :src="item.variant_image ? getFullImage(item.variant_image) : defaultPlaceholder"
                                                        @error="handleImageError" class="rounded border shadow-sm"
                                                        style="width: 45px; height: 45px; object-fit: cover;">
                                                    <div>
                                                        <div class="fw-bold text-dark text-wrap"
                                                            style="max-width: 250px;">{{ item.product_name }}</div>
                                                        <div class="text-muted" style="font-size: 0.7rem;">SKU: {{
                                                            item.variant_sku }}</div>
                                                        ...
                                                        <div class="mt-1 text-secondary fw-medium"
                                                            ...
                                                            v-else-if="item.variant_attributes">
                                                            <span
                                                                v-for="(val, key) in parseAttributes(item.variant_attributes)"
                                                                :key="key" class="me-2">[{{ key }}: {{ val }}]</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                ...
                                <div class="col-md-6">
                                    <div class="bg-light p-3 rounded border">
                                        ...
                                        <div class="d-flex justify-content-between mb-2 small text-danger"
                                            v-if="currentOrder.discount_amount > 0">
                                            <span class="text-muted fw-medium">Giảm giá Coupon <span
                                                    v-if="currentOrder.coupon_code">({{ currentOrder.coupon_code
                                                    }})</span>:</span>
                                            <strong>- {{ formatCurrency(currentOrder.discount_amount) }}</strong>
                                        </div>

                                        <div class="d-flex justify-content-between mb-2 small text-success"
                                            v-if="currentOrder.tier_discount_amount > 0">
                                            <span class="text-muted fw-medium"><i
                                                    class="bi bi-star-fill text-warning me-1"></i>Ưu đãi hạng TV:</span>
                                            <strong>- {{ formatCurrency(currentOrder.tier_discount_amount) }}</strong>
                                        </div>

                                        <div
                                            class="d-flex justify-content-between mt-3 pt-2 border-top border-2 border-dark align-items-center">
                                            <span class="fw-bold text-dark text-uppercase tracking-wide">TỔNG
                                                CỘNG:</span>
                                            <strong class="fs-4 text-brand font-oswald">{{
                                                formatCurrency(currentOrder.total_amount) }}</strong>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-4">
                                <h6 class="fw-bold text-muted text-uppercase mb-3"><i
                                        class="bi bi-clock-history me-2 text-brand"></i>Lịch sử cập nhật</h6>
                                <ul class="list-group list-group-flush border rounded custom-scrollbar-y"
                                    style="max-height: 200px; overflow-y: auto;">
                                    <li class="list-group-item d-flex justify-content-between align-items-start p-3"
                                        v-for="(history, index) in currentOrder.histories" :key="history.id"
                                        :class="{ 'bg-light': index % 2 === 0 }">
                                        <div class="ms-2 me-auto w-100">
                                            <div class="d-flex justify-content-between align-items-center w-100 mb-1">
                                                <div class="fw-bold text-dark small">
                                                    Chuyển sang <span class="badge ms-1 border shadow-sm"
                                                        :class="getOrderStatusBadge(history.new_status)">{{
                                                        formatOrderStatus(history.new_status) }}</span>
                                                </div>
                                                <span class="text-muted small"><i
                                                        class="bi bi-calendar-event me-1"></i>{{
                                                    formatDateTime(history.created_at) }}</span>
                                            </div>
                                            <div class="text-muted mt-2" style="font-size: 0.75rem;">
                                                <span class="badge bg-secondary bg-opacity-10 text-dark border me-2"><i
                                                        class="bi bi-person-fill me-1"></i>Bởi: {{
                                                    history.changer?.fullName || 'Hệ thống/Khách' }}</span>
                                                <span v-if="history.note"
                                                    class="fst-italic text-danger fw-medium d-block d-md-inline mt-2 mt-md-0"><i
                                                        class="bi bi-chat-right-text me-1"></i>"{{ history.note
                                                    }}"</span>
                                            </div>
                                        </div>
                                    </li>
                ...
                <div class="modal-body p-5 text-center bg-white" v-else>
                    <div class="spinner-border text-brand" style="width: 3rem; height: 3rem; border-width: 0.25em;"
                        role="status"></div>
                ...
                <div class="modal-footer bg-light border-top-0 rounded-bottom-4 gap-2 no-print">
                    <button type="button" class="btn btn-outline-dark rounded-pill px-4 fw-bold action-btn-hover"
                        @click="hide">Đóng</button>
                    <button type="button" class="btn btn-dark text-white rounded-pill px-4 fw-bold action-btn-hover"
                        @click="printInvoice" :disabled="!currentOrder"><i class="bi bi-printer me-2"></i> In Hóa
                        Đơn</button>
                </div>
            </div>
        </div>
...
// Tự động bắt lấy sự thay đổi của prop order từ parent (vì prop update là async)
...
.border-brand-subtle {
    border-color: rgba(0, 153, 129, 0.3) !important;
}
...
.action-btn-hover:hover {
    ...
    box-shadow: 0 6px 12px rgba(0, 153, 129, 0.25) !important;
