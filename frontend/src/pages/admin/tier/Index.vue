    ...
    <div class="container-fluid py-4" v-else>
      <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-6 text-md-end mt-3 mt-md-0 d-flex justify-content-md-end align-items-center gap-2">
          <router-link :to="{ name: 'admin-tiers-create' }"
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0 tier-table" style="width: 100%; min-width: 1100px;">
              <thead class="bg-light">
                <tr>
                  <th class="py-3 px-4 text-secondary border-0 table-column-nowrap"
                    style="width: 23%; min-width: 200px;">Hạng Thành Viên</th>
                  <th class="py-3 px-4 text-secondary border-0 text-end table-column-nowrap"
                    style="width: 18%; min-width: 180px;">Chi tiêu tối thiểu</th>
                  <th class="py-3 px-4 text-secondary border-0 text-center table-column-nowrap"
                    style="width: 18%; min-width: 180px;">Giảm giá & Lượt dùng</th>
                  <th class="py-3 px-4 text-secondary border-0 text-center table-column-nowrap"
                    style="width: 16%; min-width: 170px;">Số lần Vệ sinh/năm</th>
                  <th class="py-3 px-4 text-secondary border-0 text-center table-column-nowrap"
                    style="width: 15%; min-width: 160px;">Số Khách hàng</th>
                  <th class="py-3 px-4 text-secondary text-center border-0 table-column-nowrap"
                    style="width: 10%; min-width: 170px;">Thao tác</th>
                </tr>
              </thead>
              <tbody>
                ...
                <template v-if="isTableLoading">
                  <tr v-for="i in 3" :key="'skeleton-' + i">
                    ...
                    <td class="px-4 text-center placeholder-glow"><span class="placeholder col-6 rounded py-2"></span>
                    </td>
                    <td class="px-4 text-center placeholder-glow"><span class="placeholder col-6 rounded py-2"></span>
                    </td>
                    <td class="px-4 text-center placeholder-glow"><span class="placeholder col-6 rounded py-2"></span>
                    </td>
                    <td class="px-4 text-center placeholder-glow"><span class="placeholder col-8 rounded py-2"></span>
                    </td>
                  </tr>
                </template>
                ...
                <template v-else>
                  ...
                  <tr v-else v-for="tier in tiers" :key="tier.id" :class="{ 'bg-light': tier.min_spent == 0 }">
                    <td class="px-4 py-3">
                      <div class="d-flex align-items-center gap-3">
                        <div
                          class="position-relative shadow-sm border rounded-circle bg-white d-flex justify-content-center align-items-center overflow-hidden"
                          style="width: 45px; height: 45px; padding: 0.2rem;">
                          <!-- Sử dụng SoraImage thay cho img thường -->
                          <SoraImage :src="tier.icon" :placeholder="placeholderImg"
                            imgClass="w-100 h-100 object-fit-contain" alt="Tier Icon" />
                        </div>
                        <div>
                          <div class="fw-bold text-dark fs-6">{{ tier.name }}</div>
                          <span v-if="tier.min_spent == 0"
                            class="badge bg-secondary bg-opacity-10 text-secondary border mt-1"
                            style="font-size: 0.65rem;">Hạng Mặc định (Gốc)</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td class="px-4 text-end fw-bold text-danger">
                      ...
                      <div class="text-muted small fw-normal mt-1" v-if="tier.min_orders > 0">hoặc {{ tier.min_orders }}
                        đơn hàng</div>
                    </td>

                    <td class="px-4 text-center">
                      <span class="badge bg-success fs-6">{{ tier.discount_percent }}%</span>
                      <div class="text-muted small mt-1 fw-bold text-warning" style="color: #fd7e14 !important;">{{
                        tier.yearly_discount_quota }} lần/năm</div>
                    </td>
                    ...
                    <td class="px-4 text-center">
                      <span class="badge rounded-pill border fw-bold px-3 py-2"
                        :class="tier.users_count > 0 ? 'border-primary text-primary bg-primary bg-opacity-10' : 'border-secondary text-secondary bg-light'">
                    ...
                    <td class="px-4 text-center">
                      <div
                        class="action-buttons d-inline-flex align-items-center justify-content-center flex-wrap gap-2">
                        <router-link :to="{ name: 'admin-tiers-edit', params: { id: tier.id } }"
                          class="btn btn-sm btn-light text-primary shadow-sm border" title="Sửa">
                          <i class="bi bi-pencil-square"></i>
                        </router-link>
                        <button v-if="tier.min_spent != 0 && tier.users_count == 0"
                          class="btn btn-sm btn-light text-danger shadow-sm border" @click="confirmDelete(tier.id)"
                          title="Xóa">
                          <i class="bi bi-trash"></i>
                        </button>
                        <button v-else class="btn btn-sm btn-light text-muted shadow-sm border opacity-50"
                          title="Không thể xóa hạng đang sử dụng" disabled>
                          <i class="bi bi-trash"></i>
                        </button>
                      </div>
                    </td>
