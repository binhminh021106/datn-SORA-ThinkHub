        ...
        <ul class="nav nav-underline border-bottom mb-2 pb-1" style="flex-wrap: wrap !important; gap: 8px;">
          <li class="nav-item">
            <a class="nav-link py-2 px-3 d-flex align-items-center custom-tab" href="#" :class="{ 'active-tab': activeTab === 'all' }" @click.prevent="switchTab('all')">
              ...
              <span class="badge ms-2 rounded-pill tab-badge" :class="{'active-badge': activeTab === 'all'}">{{ totalCount }}</span>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link py-2 px-3 d-flex align-items-center custom-tab" href="#" :class="{ 'active-tab': activeTab === 'active' }" @click.prevent="switchTab('active')">
              ...
              <span class="badge ms-2 rounded-pill tab-badge" :class="{'active-badge': activeTab === 'active'}">{{ activeCount }}</span>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link py-2 px-3 d-flex align-items-center custom-tab" href="#" :class="{ 'active-tab': activeTab === 'hidden' }" @click.prevent="switchTab('hidden')">
              ...
              <span class="badge ms-2 rounded-pill tab-badge" :class="{'active-badge': activeTab === 'hidden'}">{{ hiddenCount }}</span>
            </a>
          </li>
          <li class="nav-item ms-auto">
            <a class="nav-link py-2 px-3 d-flex align-items-center custom-tab text-danger" href="#" :class="{ 'active-tab': activeTab === 'deleted' }" @click.prevent="switchTab('deleted')">
              ...
              <span class="badge ms-2 rounded-pill tab-badge" :class="{'active-badge': activeTab === 'deleted'}">{{ deletedCount }}</span>
            </a>
          </li>
        </ul>
      </div>
      ...
      <div class="card border-0 shadow-sm rounded-4 mb-4" :class="{'border-warning border-2': isReorderMode}">
        <div class="card-header bg-white border-bottom-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div class="d-flex align-items-center gap-2">
            ...
            <!-- Indicator hiển thị nạp dữ liệu ngầm mượt mà (Background Fetching) -->
            <div v-if="isFetching && !isLoading" class="spinner-border spinner-border-sm text-brand" role="status"></div>
          </div>
          ...
          <div class="d-flex align-items-center flex-wrap gap-2">
            ...
            <button v-if="isReorderMode" class="btn btn-sm btn-warning text-dark fw-bold px-4 shadow-sm py-2" @click="saveReorder" :disabled="isReorderPending">
              <span v-if="isReorderPending" class="spinner-border spinner-border-sm me-2"></span>
        ...
        <div class="card-body p-0 mt-2">
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" :class="{'table-reorder': isReorderMode}" style="table-layout: fixed; width: 100%; min-width: 1000px;">
              ...
              <tbody>
                <!-- HIỆU ỨNG SKELETON INLINE TRỰC TIẾP (Khi đang tải lần đầu) -->
                <template v-if="isLoading">
                  <tr v-for="n in 5" :key="'skeleton-row-' + n" class="skeleton-row-item">
                    <td v-if="isReorderMode" class="px-4 text-center">
                      <div class="skeleton-shimmer" style="width: 20px; height: 20px;"></div>
                    </td>
                    <td class="px-4 text-center">
                      <div class="skeleton-shimmer mx-auto" style="width: 30px; height: 18px;"></div>
                    </td>
                    <td class="px-4 py-3">
                      <div class="d-flex align-items-center">
                        <div class="skeleton-shimmer me-3" style="width: 45px; height: 45px; border-radius: 8px;"></div>
                        <div class="flex-grow-1 overflow-hidden" style="max-width: 180px;">
                          <div class="skeleton-shimmer mb-2" style="width: 85%; height: 16px;"></div>
                          <div class="skeleton-shimmer" style="width: 60%; height: 12px;"></div>
                        </div>
                      </div>
                    </td>
                    <td class="px-4">
                      <div class="skeleton-shimmer" style="width: 100px; height: 18px;"></div>
                    </td>
                    <td class="px-4">
                      <div class="d-flex gap-1">
                        <div class="skeleton-shimmer" style="width: 50px; height: 18px; border-radius: 4px;"></div>
                        <div class="skeleton-shimmer" style="width: 50px; height: 18px; border-radius: 4px;"></div>
                      </div>
                    </td>
                    <td class="px-4 text-center">
                      <div class="skeleton-shimmer mx-auto" style="width: 110px; height: 30px; border-radius: 20px;"></div>
                    </td>
                    <td class="px-4 text-center" v-if="!isReorderMode">
                      <div class="skeleton-shimmer mx-auto" style="width: 90px; height: 30px; border-radius: 6px;"></div>
                    </td>
                  </tr>
                </template>
                ...
                <tr v-else v-for="(cat, index) in displayCategories" :key="cat.id" 
                  ...
                  <td class="px-4 py-3">
                    <div class="d-flex align-items-center">
                      <!-- Xử lý lỗi nạp ảnh bằng @error Fallback thông minh tránh vỡ layout -->
...
// TANSTACK QUERY: Đọc toàn bộ danh sách danh mục (Tự động nạp từ cache)
const { data: categoriesData, isLoading, isFetching } = useQuery({
  queryKey: ['admin-categories-all'],
  queryFn: async () => {
    const res = await axios.get(`${API_URL}/admin/categories`, { headers: getHeaders() });
    return Array.isArray(res.data.data) ? res.data.data : (res.data.data?.data || []);
  },
  staleTime: 5 * 60 * 1000, // Lưu cache trong 5 phút
  keepPreviousData: true
});

// Local state để xử lý chỉnh sửa trạng thái trực quan
const categories = ref([]);

// Đồng bộ danh mục từ bộ nhớ cache của Vue Query khi có cập nhật mới
watch(categoriesData, (newVal) => {
  if (newVal) {
    categories.value = newVal.map(c => ({
      ...c, 
      localStatus: c.localStatus !== undefined ? c.localStatus : c.status, 
...
// Tải thông tin module quản trị phân quyền
const fetchSystemModules = async () => {
  try {
    ...
    if (currentModule) {
      currentPageLevel.value = currentModule.required_level;
    }
  } catch (err) {
...
// Đếm dữ liệu phản hồi nhanh trên UI
const totalCount = computed(() => categories.value.filter(c => !c.deleted_at).length);
const activeCount = computed(() => categories.value.filter(c => c.status === 'active' && !c.deleted_at).length);
const hiddenCount = computed(() => categories.value.filter(c => c.status === 'hidden' && !c.deleted_at).length);
const deletedCount = computed(() => categories.value.filter(c => c.deleted_at).length);
...
// TANSTACK MUTATION: Sắp xếp danh mục
const { mutate: mutateReorder, isPending: isReorderPending } = useMutation({
  mutationFn: async (payload) => {
    return axios.post(`${API_URL}/admin/categories/reorder`, { categories: payload }, { headers: getHeaders() });
  },
  onSuccess: () => {
...
const checkStatusChange = (cat) => { 
  cat.isStatusChanged = (cat.localStatus !== cat.status); 
};
const cancelStatusChange = (cat) => { 
  cat.localStatus = cat.status; 
  cat.isStatusChanged = false; 
};

// TANSTACK MUTATION: Cập nhật nhanh trạng thái Inline
const { mutate: mutateStatus } = useMutation({
  mutationFn: async ({ id, formData }) => {
    return axios.post(`${API_URL}/admin/categories/${id}`, formData, { headers: getHeaders() });
  },
...
const saveCategoryStatus = (cat) => {
  ...
  mutateStatus({ id: cat.id, formData });
};
...
// TANSTACK MUTATION: Đưa vào thùng rác
const { mutate: mutateDelete } = useMutation({
  mutationFn: async (id) => {
    return axios.delete(`${API_URL}/admin/categories/${id}`, { headers: getHeaders() });
  },
  onSuccess: () => {
...
const confirmDelete = (id, name) => {
  Swal.fire({ 
    title: 'Xóa danh mục?', 
    text: `Danh mục "${name}" sẽ bị đưa vào thùng rác!`, 
    icon: 'warning', 
