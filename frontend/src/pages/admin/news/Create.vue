            ...
            <form @submit.prevent="handleSave" autocomplete="off">
                <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <button type="button" class="btn btn-light border px-4 py-2 fw-semibold shadow-sm text-nowrap"
                            @click="router.push('/admin/news')">Hủy bỏ</button>
                        <button type="submit"
                            class="btn btn-brand btn-brand-solid px-4 py-2 fw-bold shadow-sm text-nowrap"
                            :disabled="isLoading">
                    ...
                    <div class="col-lg-8">
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-body p-4">
                                <h6 class="fw-bold mb-4 text-dark"><i class="bi bi-journal-text text-brand me-2"></i>Nội
                                    dung bài viết</h6>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text"
                                        class="form-control form-control-lg border rounded-3 bg-white shadow-sm fs-5 fw-bold"
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="input-group shadow-sm rounded-3 overflow-hidden border">
                                        ...
                                        <input type="text" class="form-control border-0 bg-white"
                                            v-model="formData.slug" placeholder="tu-dong-tao-tu-tieu-de">
                                    </div>
                                    <small class="text-muted mt-1 d-block">Để trống hệ thống sẽ tự động tạo từ tiêu
                                        đề.</small>
                                </div>

                                <!-- Mô tả ngắn -->
                                <div class="mb-4">
                                    <label class="form-label fw-semibold text-dark required">Mô tả ngắn
                                        (Excerpt)</label>
                                    <textarea class="form-control border rounded-3 bg-white shadow-sm" rows="3"
                                        v-model="formData.excerpt"
                                        placeholder="Tóm tắt ngắn gọn nội dung bài viết (sẽ hiển thị ở trang chủ và danh sách tin tức)..."
                                        required></textarea>
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <label class="form-label fw-semibold text-dark required mb-0">Nội dung chi
                                            tiết</label>
                                    ...
                                    <div class="editor-container shadow-sm rounded-4 position-relative">
                                        <QuillEditor v-if="!isHtmlMode" theme="snow" toolbar="full"
                                            v-model:content="formData.content" contentType="html"
                                            placeholder="Bắt đầu soạn thảo nội dung..." />
                                        <textarea v-else class="form-control font-monospace p-3" rows="15"
                                            v-model="formData.content"
                                            placeholder="<h1>Tiêu đề</h1><p>Nội dung HTML...</p>"
                                            style="min-height: 400px; background-color: #2d2d2d; color: #f8f8f2; border: none;"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-body p-4 text-center">
                                <h6 class="fw-bold text-start mb-3 text-dark"><i
                                        class="bi bi-image text-brand me-2"></i>Ảnh đại diện</h6>
                                <div class="image-upload-box bg-white border border-2 border-dashed border-secondary rounded-4 p-3 position-relative shadow-sm"
                                    ...
                                    style="cursor: pointer; min-height: 200px; display: flex; align-items: center; justify-content: center; flex-direction: column; transition: 0.3s;">
                                    <img v-if="previewImage" :src="previewImage"
                                        class="img-fluid rounded-3 object-fit-cover w-100" style="height: 180px;">
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-body p-4">
                                <h6 class="fw-bold mb-3 text-dark"><i class="bi bi-gear text-brand me-2"></i>Cài đặt
                                    chung</h6>

                                <div class="mb-3">
                                    <label class="form-label fw-semibold text-dark text-sm">Trạng thái hiển thị</label>
                                    <select class="form-select border shadow-sm rounded-3 bg-white py-2 fw-medium"
                                        v-model="formData.status" :class="getStatusColor(formData.status)">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select border shadow-sm rounded-3 bg-white py-2"
                                ...
                                <div class="mb-0">
                                    <label class="form-label fw-semibold text-dark text-sm required">Bút danh (Tác
                                        giả)</label>
                                    <input type="text" class="form-control border shadow-sm rounded-3 bg-white py-2"
                                        v-model="formData.author_name" placeholder="Tên tác giả hiển thị..." required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-body p-4">
                                <h6 class="fw-bold mb-3 text-dark"><i class="bi bi-search text-brand me-2"></i>Tối ưu
                                    tìm kiếm (SEO)</h6>

                                <div class="mb-3">
                                    <label class="form-label fw-semibold text-dark text-sm">Meta Title</label>
                                    <input type="text"
                                        class="form-control form-control-sm border shadow-sm rounded-3 bg-white py-2"
                                        v-model="formData.meta_title" placeholder="Mặc định lấy tiêu đề bài viết...">
                                </div>
                                ...
                                <div class="mb-0">
                                    ...
                                    <textarea
                                        class="form-control form-control-sm border shadow-sm rounded-3 bg-white py-2"
                                        rows="3" v-model="formData.meta_description"
                                        placeholder="Mặc định lấy mô tả ngắn..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
const handleSave = async () => {
    ...
    try {
        await adminApiClient.post('/news', submitData);
...
.btn-brand-solid:disabled {
    background-color: #a5d6cd !important;
    cursor: not-allowed;
}
