<template>
  <div class="brand-index-wrapper">
    ...
    <div v-if="isPageLoading" class="d-flex flex-column justify-content-center align-items-center w-100"
      ...
      <div class="row mb-4 align-items-center">
        ...
        <div class="col-md-6 text-md-end mt-3 mt-md-0 d-flex justify-content-md-end align-items-center gap-3 flex-wrap">
          <router-link :to="{ name: 'admin-brands-create' }"
            class="btn btn-brand px-4 py-2 fw-bold shadow-sm text-white rounded-pill" v-if="!isReorderMode">
      ...
      <div class="card border-0 shadow-sm rounded-4 mb-4" :class="{ 'border-warning border-2': isReorderMode }">
        <div
          ...
          <div class="d-flex align-items-center gap-2">
            ...
            <button v-if="isReorderMode" class="btn btn-sm btn-success text-white fw-bold px-4 shadow-sm py-2"
              @click="saveReorder" :disabled="isSavingOrder">
            ...
            <div class="search-box position-relative" style="width: 280px; max-width: 100%;" v-show="!isReorderMode">
              <input type="text" class="form-control form-control-sm rounded-pill pe-5 shadow-sm bg-light border-0 py-2"
                v-model="searchQuery" @input="currentPage = 1" placeholder="Tìm tên thương hiệu...">
        ...
        <div class="card-body p-0 mt-2">
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0"
              style="table-layout: fixed; width: 100%; min-width: 800px;" :class="{ 'table-reorder': isReorderMode }">
              <thead class="bg-light">
                <tr>
                  ...
                  <th class="py-3 px-4 text-secondary text-center border-0" style="width: 20%" v-if="!isReorderMode">
                    Thao tác
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                <template v-if="isTableLoading">
                  <tr v-for="i in 5" :key="'skeleton-' + i">
                    ...
                    <td class="text-center placeholder-glow" v-if="!isReorderMode"><span
                        class="placeholder col-8 rounded py-2"></span></td>
                  </tr>
                </template>
                ...
                <template v-else>
                  ...
                  <tr v-else v-for="(brand, index) in displayBrands" :key="brand.id"
                    ...
                    :draggable="isReorderMode" @dragstart="onDragStart(index, $event)"
                    @dragover.prevent="onDragOver(index, $event)" @dragenter.prevent="onDragEnter(index)"
                    @dragleave="onDragLeave(index)" @drop="onDrop(index)" @dragend="onDragEnd">
                    ...
                    <td class="px-4 py-3">
                      <div class="d-flex align-items-center">
                        ...
                        <div
                          ...
                          style="width: 55px; height: 55px; padding: 4px;">
                          <SoraImage :src="brand.logo" :placeholder="defaultImage"
                            imgClass="w-100 h-100 object-fit-contain pe-none" alt="Logo" />
                        </div>
                        <div class="overflow-hidden">
                          <div class="fw-bold text-dark fs-6 mb-1 text-truncate" :title="brand.name">{{ brand.name }}
                          </div>
                          <div class="text-muted small font-monospace"><i class="bi bi-link-45deg"></i> {{ brand.slug }}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td class="px-4 text-center" v-if="!isReorderMode">
                      <button class="btn btn-sm btn-light text-info me-2 shadow-sm border" @click="openQuickView(brand)"
                        title="Xem nhanh"><i class="bi bi-eye"></i></button>
                      <template v-if="!brand.deleted_at">
                        <router-link :to="{ name: 'admin-brands-edit', params: { id: brand.id } }"
                          class="btn btn-sm btn-light text-primary me-2 shadow-sm border" title="Sửa"><i
                            class="bi bi-pencil-square"></i></router-link>
                        <button class="btn btn-sm btn-light text-danger shadow-sm border"
                          @click="confirmDelete(brand.id, brand.name)" title="Xóa"><i class="bi bi-trash"></i></button>
                      </template>
                      <template v-else>
                        <button class="btn btn-sm btn-light text-success shadow-sm border me-2"
                          @click="restoreBrand(brand.id)" title="Khôi phục"><i
                            class="bi bi-arrow-counterclockwise"></i></button>
                        <button v-if="isHighestRole" class="btn btn-sm btn-light text-danger shadow-sm border"
                          @click="forceDeleteBrand(brand)" title="Xóa vĩnh viễn"><i
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg overflow-hidden">
          ...
          <div class="modal-body p-4 text-center" v-if="selectedBrand">
            <div
              class="mb-4 position-relative mx-auto bg-white border shadow-sm rounded-circle d-flex align-items-center justify-content-center"
              style="width: 120px; height: 120px; padding: 15px;">
              <SoraImage :src="selectedBrand.logo" :placeholder="defaultImage" imgClass="img-fluid object-fit-contain"
                style="max-height: 100%; max-width: 100%;" alt="Logo" />
            </div>
            ...
            <div class="d-flex justify-content-center gap-3 mb-4">
              <span class="badge px-3 py-2 bg-light text-dark border shadow-sm"><i
                  class="bi bi-box-seam text-brand me-1"></i> {{ selectedBrand.products_count || 0 }} Sản phẩm</span>
              <span class="badge px-3 py-2"
                :class="selectedBrand.status === 'active' ? 'bg-success bg-opacity-10 text-success border border-success' : 'bg-warning bg-opacity-10 text-warning border border-warning'">
                <i class="bi" :class="selectedBrand.status === 'active' ? 'bi-check-circle' : 'bi-eye-slash'"></i>
                {{ selectedBrand.status === 'active' ? 'Hoạt động' : 'Đang ẩn' }}
              </span>
            </div>
          ...
          <div class="modal-footer border-top-0 bg-light">
            ...
            <router-link v-if="selectedBrand && !selectedBrand.deleted_at"
              :to="{ name: 'admin-brands-edit', params: { id: selectedBrand.id } }"
              class="btn btn-brand text-white rounded-pill px-4 fw-bold" @click="closeQuickView">Sửa thông
              tin</router-link>
          </div>
        </div>
      </div>
