        ...
        <div class="col-md-4 col-lg-3">
          <div class="card border-0 shadow-sm rounded-4 text-center p-4 h-100 mb-4">
            ...
            <div class="text-start">
              ...
              <select class="form-select fw-bold shadow-sm" v-model="form.status" required
          ...
          <div class="card border-0 shadow-sm rounded-4 h-100 mb-4">
            ...
            <div class="card-header bg-white border-bottom-0 pt-3 pb-0 px-4">
              <ul class="nav nav-tabs-custom mb-3">
                ...
                <li class="nav-item">
                  <a class="nav-link d-flex align-items-center" :class="{ 'active': activeTab === 'address' }" href="#"
                    @click.prevent="activeTab = 'address'">
                    <i class="bi bi-geo-alt-fill me-2 fs-5"></i> Sổ địa chỉ
                    <span class="badge ms-2 rounded-pill"
                      :class="activeTab === 'address' ? 'bg-brand text-white' : 'bg-light text-secondary border'">{{
                      userAddresses.length }}</span>
                  </a>
                </li>
              </ul>
            </div>
            ...
            <div class="card-body p-4 pt-2">
              ...
              <form v-if="activeTab === 'info'" @submit.prevent="updateUser">
                <div class="row">
                  ...
                  <div class="col-12 mt-2 mb-3">
                    <h6 class="fw-bold text-dark border-bottom pb-2"><i class="bi bi-shield-lock me-2"></i>Đổi mật khẩu
              ...
              <div v-if="activeTab === 'address'">
                <div class="d-flex justify-content-between align-items-center mb-4">
                  <button type="button" class="btn btn-brand rounded-pill px-4 py-2 fw-bold text-white shadow-sm"
                ...
                <div class="address-list custom-scrollbar pe-2" style="max-height: 500px; overflow-y: auto;">
                  <div v-if="userAddresses.length === 0"
                    ...
                    <div
                      class="bg-white rounded-circle d-inline-flex justify-content-center align-items-center shadow-sm mb-3"
                  ...
                  <div v-else v-for="addr in userAddresses" :key="addr.id"
                    class="address-card p-4 mb-3 rounded-4 border position-relative transition-all"
                    :class="addr.is_default ? 'bg-brand bg-opacity-10 border-brand shadow-sm' : 'bg-white border-secondary-subtle'">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-start mb-3">
                      <div>
                        <h6 class="fw-bold text-dark mb-1 d-flex align-items-center flex-wrap gap-2">
                          ...
                          <span v-if="addr.is_default" class="badge bg-brand text-white ms-md-2 px-2 py-1"><i
                              class="bi bi-check-circle me-1"></i>Mặc định</span>
                        </h6>
                      </div>
                      <button v-if="!addr.is_default" @click="setDefaultAddress(addr.id)"
                        class="btn btn-sm btn-outline-secondary fw-bold rounded-pill mt-2 mt-md-0 px-3 shadow-sm hover-brand-btn"
                        :disabled="settingDefaultId === addr.id">
                        <span v-if="settingDefaultId === addr.id"
                          class="spinner-border spinner-border-sm me-1"></span>Đặt làm mặc định
                      </button>
                    </div>
                    <div class="mb-4">
                      <p class="text-dark mb-1 "><i class="bi bi-house-door text-muted me-2 fs-5 align-middle"></i>{{
                        addr.shipping_address }}</p>
                      <p class="text-muted mb-0"><i class="bi bi-map text-muted me-2 fs-5 align-middle"></i>{{
                        [addr.ward, addr.district, addr.city].filter(Boolean).join(', ') }}</p>
                    </div>
                    <div class="d-flex gap-2 border-top border-light-subtle pt-3">
                      <button
                        class="btn btn-sm btn-primary text-white rounded-pill px-4 fw-bold shadow-sm d-flex align-items-center"
                        @click="openAddressModal('edit', addr)">
                        <i class="bi bi-pencil-square me-2"></i> Sửa địa chỉ
                      </button>
                      <button v-if="!addr.is_default"
                        class="btn btn-sm btn-danger text-white rounded-pill px-4 fw-bold shadow-sm d-flex align-items-center"
            ...
            <form @submit.prevent="saveAddress">
              <div class="row g-3">
                ...
                <div class="col-12 mb-2">
                  <VietnamAddressPicker v-model:province="addrForm.city" v-model:district="addrForm.district"
                    v-model:ward="addrForm.ward" :required="true"
                ...
                <div class="col-12 mt-3" v-if="!addrForm.is_default">
                  <div
                    class="form-check form-switch p-3 bg-light rounded-3 d-flex align-items-center gap-3 border border-secondary-subtle">
                    <input class="form-check-input fs-5 m-0 cursor-pointer" type="checkbox" id="flexSwitchCheckDefault"
                      v-model="addrForm.set_as_default">
                    <label class="form-check-label fw-bold text-dark m-0 cursor-pointer"
                      for="flexSwitchCheckDefault">Đặt làm địa chỉ mặc định</label>
                  </div>
                </div>
              </div>
              <div class="text-end mt-4 pt-3 border-top">
                <button type="button" class="btn btn-light px-4 fw-bold me-2 shadow-sm" data-bs-dismiss="modal">Hủy
                  bỏ</button>
