...
class UpdateWorkShiftRequest extends FormRequest
{
    ...
    {
        // Lấy ID của work shift từ Route để ignore khi validate Unique
        $shiftId = $this->route('id') ?? $this->route('work_shift');
        ...
        $rules = [
            'name' => [
                ...
                'string', 
                'max:255', 
                Rule::unique('work_shifts', 'name')->ignore($shiftId)->whereNull('deleted_at')
            ],
